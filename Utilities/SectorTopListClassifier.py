...
NUMBER_CASE_PATTERN = re.compile(
...
def company_acronym(tokens: list[str]) -> str:
    acronym_parts = [token[0]
                     for token in tokens if token and token[0].isalpha()]
...
def load_ranked_companies(csv_path: Path) -> dict[str, RankedCompany]:
    ...
    dataframe.columns = [column.strip().lower()
...
def resolve_ranked_company(company_name: str, ranked_indexes: RankedIndexes) -> RankedCompany | None:
    ...
    for official_name, ranked_company in ranked_indexes.by_normalized_name.items():
        ...
        score = len(overlap) / max(len(input_token_set),
                                   len(official_token_set))
        if score < 0.5:
            continue
...
def apply_updates(connection: Any, updates: list[tuple[str, int]]) -> None:
    ...
    cursor.executemany(
		...
		WHERE company_id = %s
		""",
        [(category, SYSTEM_USER, company_id)
         for category, company_id in updates],
    )
    cursor.close()
    connection.commit()
