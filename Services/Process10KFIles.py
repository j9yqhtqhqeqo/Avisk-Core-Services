...
PARM_LOGFILE = (
    r'/Users/mohanganadal/Data Company/Text Processing/Programs/DocumentProcessor/Log/10KDocumentExtraction')
PARM_TENK_OUTPUT_PATH = (
    r'/Users/mohanganadal/Data Company/Text Processing/Programs/DocumentProcessor/Extracted10K/')
PARM_SOURCE_INPUT_PATH = (
    r'/Users/mohanganadal/Data Company/Text Processing/Programs/DocumentProcessor/FormDownloads/10K/')
PARM_REPROCESS_PATH = (
    r'/Users/mohanganadal/Data Company/Text Processing/Programs/DocumentProcessor/ReProcessDocHeaders/')
PARM_PROCESSED_PATH = (
    r'/Users/mohanganadal/Data Company/Text Processing/Programs/DocumentProcessor/FormDownloads/10KProcessed/')
...
def process10K():
    summary_logfile = f'{PARM_LOGFILE}10KDocumentExtractionSummary{dt.datetime.now().strftime("%Y%m%d_%H%M%S")}.txt'
    success_logfile = f'{PARM_LOGFILE}Success{dt.datetime.now().strftime("%Y%m%d_%H%M%S")}.txt'
    failure_logfile = f'{PARM_LOGFILE}Failure{dt.datetime.now().strftime("%Y%m%d_%H%M%S")}.txt'
    item_zero_logfile = f'{PARM_LOGFILE}ItemZero{dt.datetime.now().strftime("%Y%m%d_%H%M%S")}.txt'
    ...
    for year in range(PARM_BGNYEAR, PARM_ENDYEAR + 1):
        for qtr in range(PARM_BGNQTR, PARM_ENDQTR + 1):
            ...
            f_log.write(
                '\tStart Processing Year:{0}  Quarter:{1}\n'.format(year, qtr))
            f_log.flush()
            # Setup output path
            ...
            for file in file_list:
                ...
                section_processor = DocumentProcessor.tenKXMLProcessor(
                    save_results=True)
            ...
            f_log.write(
                '\t\tSuccess - Files fully Processd:{0}\n'.format(processed_all_items))
            f_log.write(
                '\t\tSuccess - Files Processd as Item[0]:{0}\n'.format(processed_zero_items))
            f_log.write(
                '\t\tFaliure  - Files Failed to Process due to errors:{0}\n'.format(error_processing))
            missed_files = total_items_submitted - processed_all_items + \
                processed_zero_items+error_processing
            if (total_items_submitted != processed_all_items+processed_zero_items+error_processing):
                f_log.write(
                    '\tALERT  - Files Not picked up for processing :{0}\n'.format(missed_files))

            f_log.write(
                '\tCompleted Processing Year:{0}  Quarter:{1}\n'.format(year, qtr))
...
def process10KHeaders():
    summary_logfile = f'{PARM_LOGFILE}10KDocumentHeaderExtractionSummary{dt.datetime.now().strftime("%Y%m%d_%H%M%S")}.txt'
    success_logfile = f'{PARM_LOGFILE}Success{dt.datetime.now().strftime("%Y%m%d_%H%M%S")}.txt'
    failure_logfile = f'{PARM_LOGFILE}Failure{dt.datetime.now().strftime("%Y%m%d_%H%M%S")}.txt'
    item_zero_logfile = f'{PARM_LOGFILE}ItemZero{dt.datetime.now().strftime("%Y%m%d_%H%M%S")}.txt'
    f_log = open(summary_logfile, 'a')
            ...
            for file in file_list:
                ...
                try:
                    ...
                    if (total_count == total_items_submitted):
                        section_processor.processDocumentHeader(
                            currenct_row_count, last_batch=True)
                    else:
                        section_processor.processDocumentHeader(
                            currenct_row_count)
                ...
                print(f'{dt.datetime.now()}' + f'Total Files Processed Sofar:' +
                      f'{total_count} of {total_items_submitted}\n')
