                ...
                for line in f:
                    if line.strip():
                        ...
                        if len(parts) == 2:
                            ...
                            existing = next((t for t in terms_data if t['Keyword'] == parts[0].strip() and t['Related Term'] == parts[1].strip()), None)
                            if not existing:
                                terms_data.append({
                                    'Keyword': parts[0].strip(),
                                    'Related Term': parts[1].strip(),
    ...
    def show_dictionary_file_info(self):
        ...
        **Instructions:** Review the keywords below and decide whether each should be **Included** or **Excluded**.
        ...
        if st.session_state.terms_loaded and st.session_state.terms_df is not None:
            st.markdown(f"**Total Terms Found:** {len(st.session_state.terms_df)}")
            ...
            with col1:
                if st.button("💾 Save Changes", type="primary"):
                    include_count, exclude_count = self.save_terms_to_files(edited_df)
                    st.success(f"✅ Saved! {include_count} terms to Include, {exclude_count} to Exclude")
            ...
            st.info("👆 Click 'Load/Reload Terms from Files' to start reviewing terms.")
