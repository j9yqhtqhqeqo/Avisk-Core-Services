...
# Main content
tab1, tab2, tab3, tab4 = st.tabs(["🏢 Select Companies", "📅 Select Years", "📥 Download", "📁 Files"])
...
with tab1:
        ...
        if st.button("🔄 Load Company List", type="primary"):
            with st.spinner("Loading companies..."):
                try:
                    ...
                    st.success(f"✅ Loaded {len(st.session_state.companies_df)} companies")
    ...
    if upload_option == "Upload Custom CSV":
        ...
        if uploaded_file:
            ...
            st.success(f"✅ Loaded {len(st.session_state.companies_df)} companies from CSV")
    ...
    if st.session_state.companies_df is not None:
        ...
        with col2:
            ...
            if 'Sector' in df.columns or 'GICS Sector' in df.columns:
                ...
                sectors = ['All Sectors'] + sorted(df[sector_col].dropna().unique().tolist())
        ...
        with col3:
            ...
            quick_select = st.selectbox(
                "Quick Select",
                ["Custom Selection", "All Companies", "Top 10", "Top 50", "Top 100", "Tech Companies", "Energy Companies"]
            )
        ...
        if quick_select == "Top 10":
            ...
            if sector_col in df.columns:
                filtered_df = filtered_df[filtered_df[sector_col].str.contains('Technology|Information', case=False, na=False)]
        elif quick_select == "Energy Companies":
            sector_col = 'Sector' if 'Sector' in df.columns else 'GICS Sector'
            if sector_col in df.columns:
                filtered_df = filtered_df[filtered_df[sector_col].str.contains('Energy', case=False, na=False)]
        ...
        # Show selected count
        st.info(f"📌 **{len(st.session_state.selected_companies)}** companies selected")
        
        # Preview selected companies
        if st.session_state.selected_companies:
            with st.expander("📋 View Selected Companies", expanded=False):
                selected_symbols = [c.split(' - ')[0] for c in st.session_state.selected_companies]
                selected_df = df[df['Symbol'].isin(selected_symbols)]
                st.dataframe(selected_df[['Symbol', 'Company']], use_container_width=True)
...
with tab2:
    ...
    with col2:
        ...
        if years_to_download:
            st.success(f"**Selected Years:** {min(years_to_download)} - {max(years_to_download)}")
            ...
            st.caption("Reports will be automatically organized into yearly folders based on their publication date.")
...
with tab3:
    ...
    with col1:
        company_count = len(st.session_state.selected_companies) if st.session_state.selected_companies else 0
    ...
    with col2:
        if st.session_state.get('years_to_download'):
            ...
            st.metric("Years Selected", f"{year_count} ({min(st.session_state.years_to_download)}-{max(st.session_state.years_to_download)})")
        else:
            st.metric("Years Selected", "All Available")
    
    with col3:
        st.metric("Sector ID", current_sector_id if enable_sector_mapping else "Not Set")
    ...
    if company_count == 0:
        warnings.append("⚠️ No companies selected. Go to 'Select Companies' tab to select companies.")
        can_download = False
    ...
    if st.button("🚀 Start Download", type="primary", use_container_width=True, disabled=not can_download):
        ...
        for idx, row in companies_to_process.iterrows():
            ...
            metric_found.metric("Reports Found", sum(r.get('reports_found', 0) for r in results))
            metric_downloaded.metric("Downloaded", len(downloader.downloaded_reports))
        ...
        with col2:
            st.metric("Reports Found", sum(r.get('reports_found', 0) for r in results))
        with col3:
        ...
        st.dataframe(st.session_state.download_results, use_container_width=True)
...
with tab4:
    ...
    if use_storage:
        ...
        output_path = Path(output_dir) if output_dir else Path('./sustainability_reports')
    ...
    if output_path.exists():
        ...
        if year_folders:
            ...
            st.success(f"Found **{total_files}** reports across **{len(year_folders)}** years")
            
            # Year filter
            selected_years = st.multiselect(
                "Filter by Year",
                options=[f.name for f in year_folders],
                default=[f.name for f in year_folders[:3]]  # Show recent 3 years by default
            )
                ...
                with st.expander(f"📅 {folder.name} ({len(pdf_files)} reports)", expanded=False):
                    if pdf_files:
                        for file in sorted(pdf_files):
                            ...
                            with col1:
                                st.text(file.name[:50] + "..." if len(file.name) > 50 else file.name)
                            with col2:
            ...
            st.info("No yearly folders found yet. Download reports to see them organized by year.")
    else:
        st.warning(f"Output directory does not exist yet. Start a download to create it.")
