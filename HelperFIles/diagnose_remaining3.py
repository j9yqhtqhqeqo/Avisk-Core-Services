...
print(f"AVGO CIK 1730168: {sub_avgo.get('name')}  tickers={sub_avgo.get('tickers')}")
...
for cik in [1444175, 1569987]:
    ...
    if sub.get('name'):
        print(f"CIK {cik}: {sub.get('name')}  tickers={sub.get('tickers')}")
        facts = get_facts(cik)
        rev = yrs(facts, 'Revenues') or yrs(facts, 'SalesRevenueNet') or yrs(facts, 'RevenueFromContractWithCustomerExcludingAssessedTax')
...
for e in assets_cd.get('units', {}).get('USD', []):
    if e.get('form','') in ('10-K','10-K405') and e.get('fp') == 'FY':
        ...
        if yr in [2016, 2017]:
            print(f"  Assets yr={yr} val={e.get('val')} accn={e.get('accn')} end={e.get('end')}")

# 5. BlackRock old CIK 1364742 - check company name and revenue details
print()
sub_blk = get_submission(1364742)
print(f"BLK CIK 1364742: {sub_blk.get('name')}  tickers={sub_blk.get('tickers')}")
...
print(f"BLK new CIK 2012383: {sub_blk_new.get('name')}  tickers={sub_blk_new.get('tickers')}")
