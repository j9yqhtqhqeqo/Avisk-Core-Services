...
cik_names = re.findall(
    r'/cgi-bin/browse-edgar\?action=getcompany&CIK=(\d+)[^"]*"[^>]*>([^<]+)</a>', r.text)
seen = set()
...
r2 = requests.get(
    ...
    headers={**SEC_HEADERS, 'Host': 'www.sec.gov'}, timeout=30)
cik_names2 = re.findall(
    r'/cgi-bin/browse-edgar\?action=getcompany&CIK=(\d+)[^"]*"[^>]*>([^<]+)</a>', r2.text)
seen2 = set()
